...
1. **Given** any directory containing a bmad project, **When** `npx bmad-method-ui install` is run, **Then** the `_bmad-ui` app is copied into the current directory with all source files intact and a printed next-steps message is shown.

2. **Given** the install completes, **When** the user follows the printed next steps, **Then** they can run `cd _bmad-ui && npm install && npm run dev` to start the UI.
...
4. **Given** an existing `_bmad-ui` directory, **When** install is run again, **Then** the CLI warns the user before overwriting and requires explicit confirmation (prompt: `_bmad-ui already exists. Overwrite? (y/N)`; default N aborts).

5. **Given** a successful install, **When** the user opens `_bmad-ui`, **Then** the folder is fully self-contained: no external workspace dependencies, no monorepo tooling required.

## Tasks / Subtasks

- [x] Create the npm CLI package at repo root (`package.json`, `bin/install.mjs`) (AC: #1, #2, #3)
  - [x] Create `package.json` at repo root with `name: "bmad-method-ui"`, `version: "0.1.0"`, `bin: { "bmad-method-ui": "bin/install.mjs" }`, `files: ["bin/", "_bmad-ui/"]`
  - [x] Create `bin/install.mjs` as an ES Module CLI script with shebang `#!/usr/bin/env node`
  - [x] Implement `install` subcommand: copy `_bmad-ui/` to `<cwd>/_bmad-ui/`
  - [x] Print next-steps message after successful copy

- [x] Implement overwrite guard (AC: #4)
  - [x] Check if `<cwd>/_bmad-ui` already exists before copying
...
- [x] Verify self-containment (AC: #5)
  - [x] Confirm `_bmad-ui/package.json` has `"private": true` and no workspace dependencies (`workspace:*` must not appear)
  - [x] Confirm `_bmad-ui/` has no `..` imports escaping the folder

- [x] Configure `files` in root `package.json` to exclude development artifacts (AC: #3, #5)
  - [x] Exclude: `_bmad/`, `_bmad-output/`, `_bmad-ui/agents/`, `infra/`, `scripts/`, `.github/`, `docs/`
  - [x] Include only: `bin/`, `_bmad-ui/`

- [x] Add `.npmignore` or rely on `files` field to keep the published package lean
  - [x] Verify with `npm pack --dry-run` that only `bin/` and `_bmad-ui/` appear
...
- [x] Test the CLI locally end-to-end
  ...
  - [x] Confirm files land in `_bmad-ui/` of temp dir
...
This story creates the **npm package wrapper** that enables `npx bmad-method-ui install`. The installed app is the **already-existing** `_bmad-ui/` directory. The CLI's only job is to copy that directory into the user's project.

**No changes to `_bmad-ui/` source code are required.** This story is about the packaging and install mechanism only.

### Architecture Decision: Root-Level `package.json`

There is currently **no `package.json` at the repository root**. This story introduces one, scoped only to the `bmad-method-ui` npm package purpose. It must NOT interfere with `_bmad-ui/package.json` (the app). Key rules:

- Root `package.json`: `name: "bmad-method-ui"`, public npm package, `"type": "module"`, no `workspaces` field.
- App `package.json` at `_bmad-ui/package.json`: unchanged, `"private": true`.
...
const dest = join(process.cwd(), '_bmad-ui', 'bmad-ui');
const src = join(__pkgDir, '_bmad-ui', 'bmad-ui');

if (existsSync(dest)) {
  const rl = createInterface({ input: process.stdin, output: process.stdout });
  rl.question('_bmad-ui already exists. Overwrite? (y/N) ', (answer) => {
    rl.close();
...
| `_bmad-ui/package.json` | **NO CHANGE** — already `"private": true`, already correct |
...
Before marking done, verify `_bmad-ui/package.json` has NO `workspace:*` references:

```bash
grep -r "workspace:" _bmad-ui/package.json
# Must return empty
```

Verify no `..` path escapes in source:
```bash
grep -r "\.\.\/" _bmad-ui/src/ || true
...
ls _bmad-ui   # should show source files
# Test overwrite guard:
node /path/to/repo/bin/install.mjs install
# Should prompt: "_bmad-ui already exists. Overwrite? (y/N)"
```
...
npm pack --dry-run
# Expect: bin/install.mjs + all _bmad-ui/** files
...
- The `bin/` directory is new at repo root — it is NOT the same as `_bmad-ui/src/`
- `_bmad-ui/` remains the source of truth for the installed app
- No changes to `_bmad-ui/` directory structure are needed
...
- Architecture: project structure shows `_bmad-ui/` as the self-contained app package [Source: architecture.md#Complete-Project-Directory-Structure]
...
- Self-containment verified: `_bmad-ui/package.json` has `"private": true`, no `workspace:*` refs; all `../` imports in `src/` are intra-src (routes → lib, types) and do not escape the app folder.
- `npm pack --dry-run` confirmed: only `bin/install.mjs` + `_bmad-ui/**` included; `_bmad/`, `_bmad-output/`, `infra/`, `scripts/` (root), `.github/`, `docs/` all excluded.
...
- [x] [Review][Patch] `_bmad-ui/` parent directory not created before `cpSync` [bin/install.mjs:36]
